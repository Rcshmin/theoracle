...
```{r eval = F, include = F}
#Building a decision tree require new concepts and skills that I previously did not have. I will keep log of my experimentation with such techniques here.

#Recursion practice

#Function that recursively makes a vector of length zero and keeps count as well

get_zero <- function(x, counter = 0){
  if(length(x) == 1){
    return(counter + 1)
  } else {
    curr_length = length(x)
    x = x[1:(length(x) - 1)]
    return(get_zero(x, counter = counter + 1))
  }
}

#Function that adds or subtracts two's then an one to a number until it is the number 10, and keeps count as well

get_ten <- function(x, counter = 0){
  if(x == 10) return(counter)
  if(x > 10){
    if(x %% 2 == 0){
      #Divisible by 2 case
      x = x - 2
      return(get_ten(x, counter = counter + 1))
    } else {
      #Not divisible by 2 case
      x = x - 1
      return(get_ten(x, counter = counter + 1))
    }
  } else {
...
get_fibonacci <- function(x, fibonacci = 2, initial = c(0, 1)){
  if(x == 1) return(0)
  if(x == 2) return(1)
  if(x == fibonacci){
    return(max(initial))
  } else {
    append = length(initial)
    initial[append + 1] = 0
    initial[length(initial)] = initial[length(initial) - 1] + initial[length(initial) - 2]
    return(get_fibonacci(x, fibonacci = fibonacci + 1, initial = initial))
  }
}
...
decision_tree_algorithm <- function(df, 
  ...
  if(any(check_purity(data), nrow(data) < min_samples, (counter - 1) == max_depth)){
    classification = classify_data(data)
    return(print(paste(classification, counter)))
  } else {
   #Recursive part
    ...
    data_above = data_g[[1]]
    data_below = data_g[[2]]
    print(paste(split_column, split_value, counter))
    #Find the answers
...
decision_tree_algorithm(df = salmon_fish, min_samples = 1, max_depth = 6)

test_tree =  rpart(type ~ weight + length, data = salmon_fish, maxdepth = 6, parms = list(split = "information"))
